...
class SearchFragment : DialogFragment(), CircularRevealAnim.AnimListener,
    ...
    var data: MutableList<String> = arrayListOf("火星", "爱情公寓", "666", "笑cry", "漫威",
            "清新", "匠心", "VR", "心理学", "舞蹈", "品牌广告", "粉丝自制", "电影相关", "萝莉", "魔性"
            , "第一视角", "教程", "土鳖", "奥斯卡", "燃", "冰与火之歌", "温情", "线下campaign", "公益")
    ...
    private fun setData() {
        searchAdapter = SearchAdapter(activity!!, data as ArrayList<String>)
        searchAdapter.setOnDialogDismissListener(object : SearchAdapter.onDialogDismiss {
            override fun onDismiss() {
                hideAnim()
            }
        })
        val manager = FlexboxLayoutManager(context)
        manager.flexDirection = FlexDirection.ROW //设置主轴排列方式
        manager.flexWrap = FlexWrap.WRAP //设置是否换行
